...
const toString = function (x) {
    ...
    if (x instanceof Uint8Array) {
        if (isValidUTF8(x)) {
            ...
            return `b64"${exports.bytesToBase64(x)}"`;
        }
    }
    if (typeof x === 'string') {
 ...
 */
const bitsToString = (s) => sjcl_1.default.codec.utf8String.fromBits(s);
/**
...
exports.bytesToBase64 = function (bytes) {
    return tweetnacl_util_1.default.encodeBase64(bytes)
        .replace(/=+$/, '')
        .replace(/\+/g, '-')
        .replace(/\//g, '_');
};
...
const bytesToBits = function (arr) {
    ...
    return sjcl_1.default.codec.base64.toBits(tweetnacl_util_1.default.encodeBase64(arr));
};
/**
...
const bitsToBytes = function (arr) {
    // See https://github.com/bitwiseshiftleft/sjcl/issues/344 for why
    // we cannot just use sjcl.codec.bytes.toBits.
    return tweetnacl_util_1.default.decodeBase64(sjcl_1.default.codec.base64.fromBits(arr));
};
/**
...
const setJSONFieldV2 = function (obj, key, valBytes) {
    if (isValidUTF8(valBytes)) {
        ...
        obj[key + '64'] = exports.bytesToBase64(valBytes);
    }
};
/**
...
const keyedHash = function (keyBits, dataBits) {
    const hash = new sjcl_1.default.misc.hmac(keyBits, sjcl_1.default.hash.sha256);
...
const keyedHash2 = function (keyBits, d1Bits, d2Bits) {
    ...
    return keyedHash(keyBits, sjcl_1.default.bitArray.concat(h1Bits, h2Bits));
};
...
const encrypt = function (keyBits, textBits) {
    ...
    const dataBytes = tweetnacl_1.default.secretbox(textBytes, nonceBytes, keyBytes);
...
const decrypt = function (keyBits, ciphertextBits) {
    ...
    let textBytes = tweetnacl_1.default.secretbox.open(dataBytes, nonceBytes, keyBytes);
    if (!textBytes) {
...
const bindForRequest = function (rootSigBits, dischargeSigBits) {
    if (sjcl_1.default.bitArray.equal(rootSigBits, dischargeSigBits)) {
        return rootSigBits;
    }
    ...
    */
    addThirdPartyCaveat(rootKeyBytes, caveatIdBytes, locationStr) {
        const cav = {
            ...
            _vidBits: encrypt(this._signatureBits, makeKey(bytesToBits(requireBytes(rootKeyBytes, 'Caveat root key')))),
            _locationStr: maybeString(locationStr),
        };
        this._signatureBits = keyedHash2(this._signatureBits, cav._vidBits, cav._identifierBits);
    ...
    */
    verify(rootKeyBytes, check, discharges = []) {
        ...
        discharges.forEach((dm, i) => {
            if (used[i] === 0) {
                throw new Error(`discharge macaroon ${toString(dm.identifier)} was not used`);
            }
            if (used[i] !== 1) {
                // Should be impossible because of check in verify, but be defensive.
                throw new Error(`discharge macaroon ${toString(dm.identifier)} was used more than once`);
            }
        });
    }
    _verify(rootSigBits, rootKeyBits, check, discharges, used) {
        ...
        this._caveats.forEach(caveat => {
            if (caveat._vidBits) {
                ...
                for (di = 0; di < discharges.length; di++) {
                    dm = discharges[di];
                    if (!sjcl_1.default.bitArray.equal(dm._identifierBits, caveat._identifierBits)) {
                        continue;
                    }
                    found = true;
                ...
                if (!found) {
                    throw new Error(`cannot find discharge macaroon for caveat ${toString(caveat._identifierBits)}`);
                }
        ...
        if (!sjcl_1.default.bitArray.equal(boundSigBits, this._signatureBits)) {
    ...
    */
    _exportAsJSONObjectV1() {
        const obj = {
            ...
            signature: sjcl_1.default.codec.hex.fromBits(this._signatureBits),
        };
        ...
        if (this._caveats.length > 0) {
            obj.caveats = this._caveats.map(caveat => {
                ...
                if (caveat._vidBits) {
                    ...
                    caveatObj.vid = sjcl_1.default.codec.base64.fromBits(caveat._vidBits, true, true);
...
/**
  ...
  @alias module:macaroon
*/
exports.importMacaroon = function (obj) {
    if (typeof obj === 'string') {
        obj = exports.base64ToBytes(obj);
    }
...
/**
  ...
  @alias module:macaroon
*/
exports.importMacaroons = function (obj) {
    if (typeof obj === 'string') {
        obj = exports.base64ToBytes(obj);
    }
    if (obj instanceof Uint8Array) {
...
const importJSONV1 = function (obj) {
    const caveats = obj.caveats && obj.caveats.map(jsonCaveat => {
        ...
        if (jsonCaveat.vid) {
            caveat.vidBytes = exports.base64ToBytes(requireString(jsonCaveat.vid, 'Caveat verification id'));
        }
        return caveat;
    });
    return new Macaroon({
...
function v2JSONField(obj, key, required) {
    if (obj.hasOwnProperty(key)) {
        return stringToBytes(obj[key]);
    }
    const key64 = key + '64';
    if (obj.hasOwnProperty(key64)) {
        return exports.base64ToBytes(obj[key64]);
    }
    if (required) {
...
/**
  ...
  @alias module:macaroon
*/
exports.newMacaroon = function ({ identifier, location, rootKey, version }) {
    ...
    return new Macaroon({
        ...
        signatureBytes: bitsToBytes(keyedHash(makeKey(bytesToBits(rootKeyBytes)), bytesToBits(identifierBytes))),
    });
};
/**
  ...
  @alias module:macaroon
*/
exports.dischargeMacaroon = function (macaroon, getDischarge, onOk, onError) {
    ...
    const dischargeCaveats = (m) => {
        ...
        for (i = 0; i < m._caveats.length; i++) {
            ...
            getDischarge(firstPartyLocation, cav._locationStr, bitsToBytes(cav._identifierBits), dischargedCallback, dischargedErrorCallback);
            pendingCount++;
        }
        if (pendingCount === 0) {
            onOk(discharges);
